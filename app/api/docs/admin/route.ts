...
export async function PATCH(req: Request) {
  try {
    ...
    if (parsed.content !== undefined) {
      ...
      const versionsResp = await supabase.from('docs_versions').select('version_number').eq('docs_id', data.id);
      const versions = (versionsResp.data as { version_number?: number }[] | null) ?? [];
      const maxVersion = versions.length ? Math.max(...versions.map((v) => v.version_number || 0)) : 0;
      const nextVersion = maxVersion + 1;
      await supabase.from('docs_versions').insert({ docs_id: data.id, version_number: nextVersion, content: parsed.content });
    }
    return NextResponse.json({ data });
  } catch (err) {
    return NextResponse.json({ error: (err as Error)?.message || String(err) }, { status: 403 });
  }
}
